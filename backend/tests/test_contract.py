...
Each test function covers one module. Cases are tables.
...
def test_scheduler():
    # (desc, activities, start_hour, end_hour, min_scheduled, must_contain, must_not)
    cases = [
    ...
    for desc, acts, sh, eh, min_n, must, must_not in cases:
        result = schedule_day(acts, DAY, day_start_hour=sh, day_end_hour=eh)
        ids = [sa.activity.id for sa in result.activities]
        assert len(result.activities) >= min_n, f"{desc}: >= {min_n}, got {len(result.activities)}"
        for m in must:
...
def test_resilience():
    # (desc, activities, expected_comparison)
    tight_acts = [_sched("a", (35.71, 139.79), "culture", 9, 120),
                  _sched("b", (35.72, 139.80), "culture", 11, 120, travel=10)]
    spaced_acts = [_sched("a", (35.71, 139.79), "culture", 9, 60),
                   _sched("b", (35.68, 139.71), "nature", 14, 60, travel=20)]
    empty_acts = []

    tight = compute_resilience(Itinerary(destination="T", days=[
        ItineraryDay(day_index=0, date=DAY, activities=tight_acts)]))
    spaced = compute_resilience(Itinerary(destination="S", days=[
        ItineraryDay(day_index=0, date=DAY, activities=spaced_acts)]))
    empty = compute_resilience(Itinerary(destination="E", days=[
        ItineraryDay(day_index=0, date=DAY, activities=empty_acts)]))
...
def test_repair():
    base = Itinerary(destination="T", days=[ItineraryDay(day_index=0, date=DAY, activities=[
        _sched("target", (35.71, 139.79), "culture", 10, 90),
        _sched("lunch", (35.69, 139.70), "food", 12, 60, travel=30)])])

    # (desc, activity_id, pool, expect_success, expect_degraded, expect_type)
    cases = [
        ("no alternative → graceful degradation", "target", [], True, True, "removed"),
        ("non-existent activity → failure",        "ghost",  [], False, True, None),
    ]
    for desc, aid, pool, exp_ok, exp_deg, exp_type in cases:
        r = repair_itinerary(base, aid, "closure", "closed", candidate_pool=pool)
...
def test_chaos_end_to_end():
    base = Itinerary(destination="T", days=[ItineraryDay(day_index=0, date=DAY, activities=[
        _sched("target", (35.71, 139.79), "culture", 10, 90),
        _sched("lunch", (35.69, 139.70), "food", 12, 60, travel=30)])])
    result = trigger_chaos(base, candidate_pool=[])
...
def test_planner_mock():
    # (desc, destination, interests, min_activities)
    cases = [
        ("Tokyo mock",   "Tokyo",  ["culture", "food"], 1),
        ("Generic fallback", "Anywhere", [], 1),
    ]
    for desc, dest, interests, min_n in cases:
        result = _mock_activities(dest, interests)
        assert len(result["activities"]) >= min_n, f"{desc}: got {len(result['activities'])}"


# ── Haversine ────────────────────────────────────────────────────────────

def test_haversine():
    # (desc, coord1, coord2, expected_km_or_None)
    cases = [
        ("same point",           (0, 0),       (0, 0),       0.0),
        ("Senso-ji to Gyoen",    (35.7148, 139.7967), (35.6852, 139.7100), None),
    ]
    for desc, c1, c2, expected in cases:
